...
public class NonPlayerCharacter implements Character {
     ...
     * @param posy
     */
    public NonPlayerCharacter(int posx, int posy) {
        this.posx = posx;
        this.posy = posy;
        this.health=20;
        this.damage=5;
                ...
                if(i>=0&&j>=0&&i<map.length&&j<map[0].length){//check that courdinates being tested are in bounds
                    if(charMap[i][j]!=null){
                        if(charMap[i][j].isPc()) {//this is the pc.
                            ...
                            if(map[posy+yDiff][posx+xDiff]!='i'){
            ...
            for (int j = 0; j < charMap[0].length; j++) {
                ...
                if (charMap[i][j] != null) {
                    ...
                    if (charMap[i][j].isPc()) {
                        ...
                        if (this.canSeePc(charMap, map)) {//we can see the pc, so either move towards it, or attack it.
                            //straight up
                            if (yDiff == -1 && xDiff == 0) {
                                if (map[posy - 1][posx]!='i') {
                                    if (charMap[posy - 1][posx] == null) {
                                        charMap[posy - 1][posx] = this;
                                        charMap[posy][posx] = null;
                                        posy -= 1;
                                        return true;
                                    }else if(charMap[posy - 1][posx].isPc()){
                            ...
                            //up to the right
                                ...
                                if (map[posy - 1][posx + 1]!='i') {
                                    if (charMap[posy - 1][posx + 1] == null) {
                                        ...
                                        posx += 1;
                                        return true;
                                    }else if(charMap[posy - 1][posx+1].isPc()){
                                        charMap[posy-1][posx+1].hitFor(damage,charMap);//hit the pc.
                                    }
                                }
                            }
                            //up to the left
                            ...
                            //to the left
                                ...
                                if (map[posy][posx - 1]!='i') {
                                    if (charMap[posy][posx - 1] == null) {
                                        charMap[posy][posx - 1] = this;
                                        charMap[posy][posx] = null;
                                        posx -= 1;
                                        return true;
                                    }else if(charMap[posy][posx-1].isPc()){
                                        charMap[posy][posx-1].hitFor(damage,charMap);//hit the pc.
                                    }
                                }
                            }
                            //lower left
        ...
        switch (random) {
            case 0://up and to the left
                if(posy-1>=0&&posx-1>=0) {//bounds check
                    if (map[posy - 1][posx - 1] !='i') {//check if terrain is room
